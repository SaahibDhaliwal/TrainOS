...
// CHANGE DEPENDING ON TRACK
    ...
    train13[1] = &track[30];  // b15
    train13[2] = &track[67];  // e4
    // train13[2] = &track[50];  // d3
    train13[3] = &track[47];  // c16

    train13[4] = &track[71];  // e8
    train13[5] = &track[78];  // b14
...
void TrainManager::processTrainRequest(char* receiveBuffer, uint32_t clientTid) {
    ...
    switch (command) {
        ...
        case Command::MAKE_RESERVATION: {
            ...
            if (!curTrain->path.empty() && *curTrain->path.front() == curTrain->targetNode) {
                ...
                if (!curTrain->path.empty()) {
                    ...
                    if (reservationSensor == expectedSensor) {
                        ...
                        // try to loop through the path to see if this reservation sensor would be coming up in the
                        // future
                        ...
                        for (auto it = curTrain->path.begin(); it != curTrain->path.end(); ++it) {
                            ...
                            if (curSensor == node) {
                                printer_proprietor::debugPrintF(
                                    ...
                                    "TRAIN %u YOUR RESERVATION SENSOR WAS WITHIN THE PATH, JUST NOT THE FRONT!!!! "
                                    "---------------------------------",
                                    trainIndexToNum(trainIndex));

                                // if this is happening, and the sensor is "relatively close" to the front, we can just
                                // pop the path until then?
                                ...
                                // }
                                // printer_proprietor::debugPrintF(printerProprietorTid,
                                //                                 "The PATH THAT WAS SEARCHED is currently: %s",
                                //                                 strBuff);
                                while (*(curTrain->path.begin()) != node) {
                                    curTrain->path.pop();
                                }
                                printer_proprietor::debugPrintF(printerProprietorTid, " I HAVE NOW PRUNED MY PATH");
                                break;
                            }
                        }
                    }
                    ...
                    if (nextNode->type == NodeType::BRANCH) {
                        ...
                        // if next node is 156 and player input is [right], change to 155 (nextNode->edge[DIR_STRAIGHT])
                        // if next node is 154 and player input is [right], change to 153 (nextNode->edge[DIR_STRAIGHT])
            ...
            // TODO
            //  Here, on reservation failure, we should look at the TYPE of failure to see if a train is stopped in the
        ...
        case Command::UPDATE_RESERVATION: {
            ...
            // if we plan on constantly updating reservations, we could do some checking here
        ...
        case Command::NEW_DESTINATION: {
            // dont think this is used anymore 10/08
            int trainIndex = receiveBuffer[1] - 1;
...
void TrainManager::generatePath(Train* curTrain, int targetTrackNodeIdx, int signedOffset) {
    ...
    train_server::newDestinationReply(trainTasks[trainNumToIndex(curTrain->id)], tarBox, tarNum, startBox, startNum,
                                      pathReversed);
...
void TrainManager::generatePathWithoutZone(Train* curTrain, Train* otherTrain, uint32_t curTrainZone,
    ...
    printer_proprietor::debugPrintF(printerProprietorTid, "SENDING THE STOP INFO");
    printer_proprietor::debugPrintF(printerProprietorTid, "(path without zone) SENDING THE STOP INFO");
    ...
    if (curTrain == redirectedTrain) {
        train_server::newUpdateReply(trainTasks[trainNumToIndex(redirectedTrain->id)], tarBox, tarNum, startBox,
                                     startNum, pathReversed);

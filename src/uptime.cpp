...
void print_uptime(int consoleTid) {
    uartPrintf(consoleTid, "\033[s\033[%d;%dH%s\033[u", UPTIME_START_ROW, UPTIME_START_COL, UPTIME_LABEL);
}
...
void update_uptime(int printTid, uint64_t micros) {
    ...
    uartPrintf(printTid, "\033[s\033[%d;%dH%dh %dm %d.%ds   \033[u", UPTIME_START_ROW,
